...
def prep_sample(data, is_batch=False, **kwargs):
    ...
    def prep_one_sample(data):
        ...
        # print(f"shape_array: {shape_array}")
    ...
    if is_batch:
        ...
        # print(f"data.value(): {list(data.values())}")
        # print(f'data_shape: {data_shape}')  # (2, 270, 270, 3)
        for batch_idx in range(data_shape[0]):  # in range(2)
            sub_data = {k: v[batch_idx] for k, v in data.items()}
            # print(f"sub_data: {list(sub_data.values())[0].shape}")
